...
dotfile="flv"
#savefile="/home/alfaqir/rabu/${namefile}.${dotfile}"
...
##rtmpdump -e -r rtmp://videostream.indostreamserver.com/sunniyah/ -a sunniyah -y live -W http://live.sunniyahsalafiyah.net/jwplayer/player.swf -p http://live.sunniyahsalafiyah.net -f "LNX 11,2,202,258" -o ${savefile} -V &
##	sleep 3;
	#head -n 1 $savefile | grep "ICY 200 OK" ; 
##	grep FLV ${savefile}
##done
...
rtmpdump -r rtmp://videostream.indostreamserver.com/sunniyah/ -a sunniyah -y live -W http://live.sunniyahsalafiyah.net/jwplayer/player.swf -p http://live.sunniyahsalafiyah.net -f "LNX 11,2,202,258" -o ${savefile} -V
	if [ -s "${savefile}" ];then
	echo "${savefile}" >> /tmp/rabupagi.txt
	else
		rm "${savefile}";
	fi
	Hours=`date +%H%M`
...
count=`wc -l /tmp/rabupagi.txt | awk '{print $1}'`
for a in $(eval echo {1..$count});do

file_loc=`cat /tmp/rabupagi.txt | head -n $a | tail -n 1`
