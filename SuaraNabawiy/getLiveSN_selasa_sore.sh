...
##det=`date +%m_%d_%Y`
##namefile="Hadroh_Bausadan_SuaraNabawiyLive${hariH}${blnH}${thnH}_${det}.flv"
##savefile="/home/alfaqir/raw/$namefile"
det=`date +%m_%d_%Y`
namefile="Hadroh_Bausadan_SuaraNabawiyLive"
...
path_save="$path_script/Majelis/SN/Basaudan_${blnH}_${thnH}"
...
##	sleep 60;
	rtmpdump -r rtmp://videostream.indostreamserver.com/sunniyah/ -a sunniyah -y live -W http://live.sunniyahsalafiyah.net/jwplayer/player.swf -p http://live.sunniyahsalafiyah.net -f "LNX 11,2,202,258" -o ${savefile} -V
        ...
        if [ -s "${savefile}" ];then
        echo "${savefile}" >> /tmp/selasasore.txt
        else
                rm "${savefile}";
        fi
...
count=`wc -l /tmp/selasasore.txt | awk '{print $1}'`
for a in $(eval echo {1..$count});do

file_loc=`cat /tmp/selasasore.txt | head -n $a | tail -n 1`
...
if [ -f $file_loc ] ; then
	...
	--header 'x-archive-meta-description: Ini adalah arsip Hadroh Basaudan (raw shoutcast dan LIVE Streaming) bulan '${blnH}' '${thnH}' di SuaraNabawiy.com Selasa sore' \
