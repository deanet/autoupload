...
rm -f /tmp/jumatsore.txt

curl="/usr/bin/curl"
identifier="Kajian_kitab_Risalatul_Muawanah"
...
#det=`date +%m_%d_%Y`
#namefile="Kajian_Kitab_Risalatul_Muawanah_SuaraNabawiyLive${hariH}${blnH}${thnH}_${det}.flv"
#savefile="/home/alfaqir/raw/$namefile"
det=`date +%m_%d_%Y`
namefile="Kajian_Kitab_Risalatul_Muawanah_SuaraNabawiyLive"
...
path_save="$path_script/Majelis/SN/RM_${blnH}_${thnH}"
...
#	sleep 60;
	rtmpdump -r rtmp://videostream.indostreamserver.com/sunniyah/ -a sunniyah -y live -W http://live.sunniyahsalafiyah.net/jwplayer/player.swf -p http://live.sunniyahsalafiyah.net -f "LNX 11,2,202,258" -o ${savefile} -V
        if [ -s "${savefile}" ];then
        echo "${savefile}" >> /tmp/jumatsore.txt
        else
                rm "${savefile}";
        fi
	Hours=`date +%H%M`
...
count=`wc -l /tmp/jumatsore.txt | awk '{print $1}'`
for a in $(eval echo {1..$count});do

file_loc=`cat /tmp/jumatsore.txt | head -n $a | tail -n 1`
...
if [ -f $file_loc ] ; then
	...
	--header 'x-archive-meta-description: Ini adalah arsip Kajian Kitab Risalatul Muawanah (raw shoutcast dan LIVE Streaming) bulan '${blnH}' '${thnH}' di SuaraNabawiy.com Jumat sore' \
